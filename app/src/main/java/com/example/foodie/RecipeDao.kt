...
interface RecipeDao {
    ...
    fun findRecipesByIdsAndCalories(
        recipeIds: List<Int>,
        maxCalories: Int
    ): MutableList<RecipeEntity>

    @Query("SELECT * FROM recipes WHERE recipe_id IN (:recipeIds)")
    fun getRecipesByIds(recipeIds: List<Int>): MutableList<RecipeEntity>
